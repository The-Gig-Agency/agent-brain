...
| **Role / role runner** | **AB-17 (v1):** Regime-scoped action policy modules in [`routed-role-runners.ts`](../../../src/cognitive-router/routed-role-runners.ts) — **explorer** (`explore`), **optimizer** (`prune`), **constraint** (`compound`), **adversarial** (opposing inspect inside explore + **coordinate** regime logs-first). `router-runner.ts` dispatches via `chooseRoutedRegimeAction` then shared `fixed_heuristic` fallback. |
